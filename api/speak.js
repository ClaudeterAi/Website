...
// eleven_flash_v2_5 — fastest ElevenLabs model ~75ms
const PRIMARY_VOICE_ID  = 'Zv7P8CISODgj9wDHyyI9';
const FALLBACK_VOICE_ID = 'EXAVITQu4vr4xnSDxMaL'; // Sarah — warm natural female
...
async function tts(voiceId, text, apiKey) {
  return fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`, {
    ...
    body: JSON.stringify({
      ...
      voice_settings: { stability: 0.38, similarity_boost: 0.78, style: 0.40, use_speaker_boost: true },
    }),
  });
}
...
export default async function handler(req) {
  ...
  try {
    ...
    if (!text?.trim()) return new Response(JSON.stringify({ error: 'No text' }), { status: 400, headers: cors });
